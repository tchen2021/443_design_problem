...
WP= @(WS) (550*eff) *(WS/ (sigma*Cl_maxTO*TOP*S_TOFL) )/V;
...
figure(3)
% Takeoff Distance
plot(Range,WP(Range)); hatchedline(Range,WP(Range),'m',(300-180)*pi/180); hold on;
% Landing Distance
...
S=220; % AGUESS
S_lg=S_l/1.938;
V_sTEST=sqrt(S_lg/.265);
C_lTEST=(2*W)/(rho*S*V_sTEST^2)
...
C_lTEST=(2*W)/(rho*S*V_sTEST^2)
% Speed
plot(W_S,WshOverW_P); hatchedline(W_S,WshOverW_P,'b',-60/180*pi); hold on
% Ceiling
plot(W_S,Wsh_Pceil,'g'); hatchedline(W_S,Wsh_Pceil,'g',-90/180*pi);

% Graph Parameters
xlim([0,100]); xlabel("W / S"); ylabel("W / P"); grid on; ylim([0,16]);
