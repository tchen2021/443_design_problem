...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Takeoff %%%
S_TOFL=2275; % no ground constant
...
plot(W_S,P_W_TOground_med); hatchedline(W_S,P_W_TOground_med,'m',(300-180)*pi/180); hold on; % Friction for ground
plot(W_S,P_W_TOground); hatchedline(W_S,P_W_TOground,'m',(300-180)*pi/180); hold on; % Friction for ground
...
% Graph Parameters
xlim([10,100]); xlabel("W / S"); ylabel("P/ W"); grid on; ylim([0,.8]);
...
S_TOFL=2350; % [ft] - from comp assesment no ground constant 
TOPNUM=37.5;
TOP=S_TOFL/ TOPNUM;
...
s_TOGround = 2350;         % Takeoff ground run distance with grounf constant
k1 =.0447;              % Constant k1
...
% Calculate (X/W)_TO
T_W_TOground = ( (k1 * (W_S)) / (s_TOGround * rho * Cl_maxTO) + mu_G + 0.72 * C_D_0 ) / k2;
...
s_TOG = 2500;         % Takeoff ground run distance
k1 =.0376;              % Constant k1
...
grid on;
xlim([10,100]); xlabel("W / S"); ylabel("T/ W"); grid on; ylim([0,.8]);
