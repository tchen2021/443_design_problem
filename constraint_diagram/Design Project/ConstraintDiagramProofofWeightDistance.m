...
plot(W_S,P_W_TOground,'b'); hatchedline(W_S,P_W_TOground,'b',(300-180)*pi/180); hold on; % Friction for ground
...
s_TOGround = 2500;         % Takeoff ground run distance with grounf constant
k1 =.0447;              % Constant k1
...
%jet non ground
plot(Range,TW_TO(Range),'b'); hatchedline(Range,TW_TO(Range),'b'); hold on;
plot(Range,T_W_TOground,'b' ); hatchedline(Range,T_W_TOground,'b')
%ground
...
%% Landing
S_L=2700; % [ft] - from comp assesment
S_FL=S_L/.6;
V_a=sqrt(S_FL/.3); 
V_s=V_a/1.3;
