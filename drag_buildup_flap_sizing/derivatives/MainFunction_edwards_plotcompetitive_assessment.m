...
%find the max L/D CL point for drag buildup (CLEAN)
...
%find the max L/D CL point for recon mission drag buildup 
[placeholder, idx_max(6)] = max(C_L(:)./ (C_D(:)+C_D_Recon));
CL_max(6) = C_L(idx_max(6));
CD_max(6) = C_D(idx_max(6)) + C_D_Recon;
LD_max(6) = CL_max(6)/CD_max(6);
...
%find the endurance L/D CL point for recon mission drag buildup 
[placeholder, idx_max_endurance(6)] = max(C_L(:).^1.5./(C_D(:) + C_D_Recon));
CL_endurance(6) = C_L(idx_max_endurance(6));
CD_endurance(6) = C_D(idx_max_endurance(6)) + C_D_Recon;
LD_endurance(6) = CL_endurance(6)/CD_endurance(6);

%displaying results
%Displaying Results
disp('Drag Buildup L/D Results')
fprintf('\nBEST RANGE L/D:\nL/D (CLEAN): %4.2f\nC_L: %5.4f\nC_D: %5.4f', LD_max(1), CL_max(1), CD_max(1));
fprintf('\n\nL/D (ATTACK): %4.2f\nC_L: %5.4f\nC_D: %5.4f', LD_max(5), CL_max(5), CD_max(5));
fprintf('\n\nL/D (Recon): %4.2f\nC_L: %5.4f\nC_D: %5.4f', LD_max(6), CL_max(6), CD_max(6));

fprintf('\n\n\nBEST ENDURANCE L/D:\nL/D (CLEAN): %4.2f\nC_L: %5.4f\nC_D: %5.4f', LD_endurance(1), CL_endurance(1), CD_endurance(1));
