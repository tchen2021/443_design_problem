...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Takeoff %%%
S_TOFL=2250; % no ground constant CHANGE
...
s_TOGround = 2250;         % Takeoff ground run distance CHANGE

k1 =.0376;              % Constant k1
k2 =ip*((1/10)^(1/3)) ;    %proppeller disk loading range from 10-30  
...
WPtake = @(WS) ( (k1 * (WS)) / (s_TOGround * rho0 * Cl_maxTO) + mu_Gmin + 0.72 * C_D_0 ) / k2;
...
figure(1)
% Takeoff Distance
% plot(Range,WP(Range),'b'); hatchedline(Range,WP(Range),'b',(300-180)*pi/180,.4,-0.010,.01); hold on;
plot(Range,WPtake(Range),'b'); hatchedline(Range,WPtake(Range),'b',45,.4,-0.005,.02); hold on;
% -0.005,.024
% plot(W_S,P_W_TOground_med,'b'); hatchedline(W_S,P_W_TOground_med,'b',(300-180)*pi/180,.3,-0.014,.01); hold on; % Friction for ground
...
% Adjust these values:
spacing2 = 0.002;  % Decrease for more frequent hatches
hatch_length = .8;  % Decrease for shorter hatches
angle2 = (290 - 180) * pi / 180;  % Keep the angle the same

%%% Graph %%% 
plot(WS*ones(1,100),linspace(0,1.4,100),'r');hatchedline(WS * ones(1, 100), linspace(0, 1.4, 100), 'r-', angle2, spacing2, hatch_length); hold on;

% hatchedline(WS*ones(1,100),linspace(0,1.4,100),'r-',(300-180)*pi/180)
...
WS= ((rho*Cl_maxL*V_a^2)/2)/WeightRatio;
%%% Graph %%% 
% plot(WS*ones(1,100),linspace(0,1.4,100));hatchedline(WS * ones(1, 100), linspace(0, 1.4, 100), 'r-', angle2, spacing2, hatch_length); hold on;
...
%%% Graph %%%
plot(W_S,WshOverW_P,'g'); hatchedline(W_S,WshOverW_P,'g',theta,.3,-0.005,.027);hold on;
...
%%% Graph %%%
plot(W_S,Psh_Wceil,'k'); hatchedline(W_S,Psh_Wceil,'k',theta,.24,-0.007,.025);hold on
% -0.005,.024
...
% 
%P_W_jetdata = load('WPL_WS_turboprop_WPL4000_WPL3000.xlsx');
 %P_Wjet_data= P_W_jetdata.WTO_vs_WS_turboprop_profile1;
% 
% 
%W_S_prop5 = P_Wjet_data(:,1);
% 
% P_Wpropnew = 1600./(P_Wjet_data(:,2));

% filename = 'WPL_WS_turboprop_WPL4000_WPL3000.xlsx';
% 
% % Read the data from the file
% data = readmatrix(filename);
% 
% % Extract the first column and third column (adjust rows if headers exist)
% x = VarName2; % Column 2 
% y = 1600./VarName9; %
% 
% % yt = yticks; % Get current y-axis ticks
...
% 
%  P_W_propdata = load('T_W_jet.txt');
% P_Wprop_data= P_W_propdata.T_W_jet;
% % 
%  W_S_jetdata = load('W_S_jet.mat'); 
%  W_Sjet_data= W_S_jetdata.W_S_jet;
% % 
% plot(W_Sjet_data,P_Wprop_data ,'m','LineWidth',1.2); hold on
% 
...
% Graph Parameters
xlim([30,80]); xlabel("W / S [ lb / ft^{2} ] ","FontSize",26); ylabel("P/ W [ Shp / lb ] ","FontSize",26);
grid on; ylim([0.09,.165]);
...
%right side axis
% Second Y-Axis (Right)
Weightyaxis = 1600./powerweight;

yyaxis right

plot(wingload, Weightyaxis/1000,'Color',[0.75, 0.65, 0] , 'LineWidth', 1.4); hold on 
ylabel('W_{TO} (1000 lbs)');
ax=gca;
ax.YColor =[0.75, 0.65, 0] ; % Change right y-axis color to match the plot
set(gca, 'FontSize', 26);
set(groot, 'DefaultAxesFontName', 'Calibri');   % Change axes font
set(groot, 'DefaultTextFontName', 'Calibri');   % Change text font
xlim([30,80])
...
%jet non ground
% plot(Range,T_W_TOground2,'b'); hatchedline(Range,T_W_TOground2,'b',(300-180)*pi/180,1,-0.015,.02); hold on;
 plot(Range,T_W_TOground8,'b' ); hatchedline(Range,T_W_TOground8,'b',50*(pi/180),1,-0.015,.02);hold on;
%ground
...
plot(WS*ones(1,100),linspace(0,1.4,100),'r');hatchedline(WS * ones(1, 100), linspace(0, 1.4, 100), 'r-', angle2, .020, 1.2); hold on;

% % Adjust these values:
% spacing2 = 0.003;  % Decrease for more frequent hatches
% hatch_length = 1.1;  % Decrease for shorter hatches
% angle2 = (290 - 180) * pi / 180;  % Keep the angle the same
...
grid on;
xlim([15,93]); %xlabel("W / S [ lb / ft^{2} ] ","FontSize",26); ylabel("T / W [ lbf / lb] ","FontSize",26); 
grid on; ylim([0.04,.55]);
