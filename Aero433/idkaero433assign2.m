...
LD_coarse = linspace(10, 20, 7);  % Lift-to-drag ratios


SFC_coarse = linspace(0.3, 0.6, 8);  % Specific Fuel Consumption values
...
for i = 1:length(LD)
    ...
    for j = 1:length(SFC)
        ...
        G = @(Wto) ((Wto - payload - (W_f(i,j) * Wto) ) / Wto) - EmptyRatio;
        
        initial_guess2 = 120000;  % 110,000 lbs is the initial guess in the book
...
for i = 1:length(LD)
    ...
    if ismember(LD(i), LD_coarse)  % Check if LD(i) is in the coarse values
        ...
        carpettext(LD, SFC, solution2(:, :, 1)', offset, LD(i), SFC(end), ...
            ['L/D = ', num2str(LD(i))], 0*offset, 9e2*offset, 'FontSize', 9, 'Color', 'r', 'FontWeight', 'bold');
    end
end
...
for j = 1:length(SFC)
    ...
    if ismember(SFC(j), SFC_coarse)  % Check if SFC(j) is in the coarse values
        ...
        carpettext(LD, SFC, solution2(:, :, 1)', offset, LD(end), SFC(j), ...
            ['SFC = ', num2str(SFC(j))], 0.15* offset, -0.15*offset, 'FontSize', 9, 'Color', 'b', 'FontWeight', 'bold');
    end
end
