function [W_TO, W_E, W_F,MFF] = Weight_fractions_2_turboprop(c_j, W, W_pl, Range, E, V_lt, L_Dcr, L_Dlt, etap, EWF, RFF, initial_guess,p)
...
(exp((R_cr * SFC_cr)/(LD_cr * etap * 325)));
(exp((E * V_lt * SFC_lt)/(LD_lt * etap * 325)));


    if p == 1
        W(5) = (exp((Range * SFC)/(LD_cr * etap * 325)))^-1;
        if E == 0
            W(6) = 1;
        else
            W(6) =  (1 / (exp((E * V_lt * SFC)/(LD_lt * etap * 325)));
        end
        W(7) = (exp((Range * SFC)/(LD_cr * etap * 325)))^-1;
        MFF = prod(W,"all");
